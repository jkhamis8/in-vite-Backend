...
router.post('/createInvite', async (req, res) => {
  try {
    ...
    if (guestInDatabase) {
      return res.status(500).json({ error: 'Guest already Invited.' })
    }
    ...
    try {
      ...
      <p>
        <a href="${createInvite._id}" style="display:inline-block;padding:10px 20px;font-size:16px;color:#fff;background-color:#4CAF50;text-decoration:none;border-radius:5px;">RSVP Now</a>
      </p>
