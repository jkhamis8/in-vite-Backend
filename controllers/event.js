...
router.get('/getAllEvents/:userID', async (req, res) => {
  try {
    ...
    const eventObj = await Event.find({ "eventManager": { $in: userID } })
    res.json({ eventObj });
  } catch (error) {
    console.log(error);
  }
...
router.put('/addUserInEvent', async (req, res) => {
  try {
    ...
    await Event.findByIdAndUpdate(eventID, { $push: { representatives: userID } })
    res.status(200).json({ 'done': 'done' });
  } catch (error) {
    console.log(error);
  }
...
router.put('/removeUserFromEvent/', async (req, res) => {
  try {
    const eventID = req.body.eventID
    const userID = req.body.userID
    await Event.findByIdAndUpdate(eventID, { $pull: { representatives: userID } })
    res.status(200).json({ 'done': 'done' });
  } catch (error) {
    console.log(error);
  }
